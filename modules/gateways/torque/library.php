...
class Torque_rpc
{
    ...
    public function __construct($pUrl, $pUser = null, $pPass = null) {
		...
		$gatewayx = getGatewayVariables("torque");
  ...
  /*  public function setParametersStructure($pParametersStructure)
   ...
   private function request($pMethod, $pParams)
        ...
        if (isset($responseDecoded['error']))
        {
            $errorMessage = 'Request have return error: ' . $responseDecoded['error']['message'] . '; ' . "\n" .
                ...
                if ($responseDecoded['error']['message'] == "Method not found") {
                	$errorMessage .= " Check the daemon hostname and daemon port settings in the Torque Pyament Gateway WHMCS config.";
                }
    ...
    public function make_uri($address, $amount_xtc, $recipient_name = null, $description = null)
    {
        // If I pass 1, it will be 0.0000001 xtc. Then
        $new_amount = $amount_xtc * 1000;
    ...
    public function transfer($amount_xtc, $address, $mixin = 4)
    {
        $new_amount = $amount_xtc  * 100;
