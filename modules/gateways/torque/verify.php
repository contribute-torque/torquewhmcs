...
function verify_payment($payment_id, $amount, $amount_xtc, $invoice_id, $fee, $status, $gatewaymodule, $hash, $secretKey, $currency){
	...
	if ($payment_id !="") {
		...
		if ($hash != md5($invoice_id . $payment_id . $amount_xtc . $secretKey)) {
		...
		// send each torque tx in the mempool to handle_whmcs
		if ($check_mempool) {
			$get_payments_method = $torque_daemon->get_transfers('pool', true);
			foreach ($get_payments_method["pool"] as $tx => $transactions) {
				...
				if(isset($txn_amt)) {
					return handle_whmcs($invoice_id, $amount_xtc, $txn_amt, $txn_txid, $txn_payment_id, $payment_id, $currency, $gatewaymodule);
				}
			}
		}
		// send each torque tx to handle_whmcs
...
function handle_whmcs($invoice_id, $amount_xtc, $txn_amt, $txn_txid, $txn_payment_id, $payment_id, $currency, $gatewaymodule) {
	$amount_atomic_units = $amount_xtc * 100;

	//check if torque tx already exists in whmcs
	...
	if ($txn_payment_id == $payment_id) {
		if (!$transaction_exists) {
			...
			$fiat_paid = xtc_to_fiat($txn_amt, $currency);
			add_payment("AddInvoicePayment", $invoice_id, $txn_txid, $gatewaymodule, $fiat_paid, $txn_amt / 100, $payment_id, $fee);
		}
		...
		if ($txn_amt * 1.02 >= $amount_atomic_units) {
			...
			return "Error: Amount " . $txn_amt / 100 . " XTC too small. Please send full amount or contact customer service. Transaction ID: " . $txn_txid . ". Payment ID: " . $payment_id;
		}
	}
}


function add_payment($command, $invoice_id, $txn_txid, $gatewaymodule, $fiat_paid, $amount_xtc, $payment_id, $fee) {
	$postData = array(
		'action' => $command,
...
$vefiry = verify_payment($payment_id, $amount, $amount_xtc, $invoice_id, $fee, $status, $gatewaymodule, $hash, $secretKey, $currency);
echo $vefiry;
